...
public class Shell {
    ...
    public static String PROJECT_HOMEPAGE_URL = "https://asdwiki.isus.emc.com:8443/display/PV/ECS+Shell";

    private Output output;
    private Input input;
    private String appName;

    private String batchFile;  // if this is not null, run the file, then exit
    ...
    public void commandLoop() throws IOException {
        ...
        while (!command.trim().equals("exit")) {
            try {
                if (batchFile != null) {
                    command = "!rs " + batchFile;
                    batchFile = null;
                } else {
                    command = input.readCommand(path);
                }
                command = com.emc.ecs.Settings.getInstance().parseVariables(command);
    ...
    private static final String HINT_FORMAT = "This is %1$s \n" +
            "For more information on the Shell, enter '?help' \n" +
            "Enter 'exit' for backing up one level ";
